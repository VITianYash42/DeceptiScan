...
function showToast(message){
    ...
    toast.className = `${color} text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 min-w-[280px] animate-toast-in`;
    ...
    close.className = "ml-2 opacity-70 hover:opacity-100 transition cursor-pointer";
...
function playAudio(type) {
    try {
        ...
        sound.volume = 0.5;
        sound.play().catch(e => console.log("Audio file missing."));
    } catch (e) {}
}

// 🚨 UPDATED: Handles Glitch vs Instant Feedback
async function sendResult(id, success, pointsAwarded, explanationStr) {
...
// Helper to save to DB and trigger UI
async function saveAndShowFeedback(id, success, pointsAwarded, explanationStr, scenario) {
    await fetch("/api/process-email",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ scenario_id: id, success: success, points: pointsAwarded })
    });

    let pointsStr = pointsAwarded > 0 ? `+${pointsAwarded}` : pointsAwarded;
    showFeedbackOverlay(success, pointsStr, explanationStr, scenario.is_phish);
}

// Unified Scoring & Bonus Calculation
function processAction(scenario, isSuccess, basePoints) {
    let timeTaken = (Date.now() - scenarioStartTime) / 1000;
    let timeBonus = 0;
    let streakBonus = 0;

    if (isSuccess) {
        currentStreak++;
        if (timeTaken < 5) timeBonus = 15;
        streakBonus = (currentStreak > 1) ? (currentStreak * 5) : 0;
    } else {
        currentStreak = 0; 
    }
    ...
    let bonusText = "";
    if (timeBonus > 0) bonusText += `⏱️ Speed Bonus (+${timeBonus})! `;
    if (streakBonus > 0) bonusText += `🔥 Streak x${currentStreak} (+${streakBonus})!`;
    
    let finalExplanation = scenario.explanation;
    if (bonusText !== "") {
        finalExplanation = `<div class="mb-3 text-green-700 font-black tracking-wide bg-green-100 inline-block px-3 py-1 rounded-lg border border-green-300">${bonusText}</div><br>` + finalExplanation;
    }

    sendResult(scenario.id, isSuccess, totalPoints, finalExplanation);
    updateStreakUI();
}

function clickLink(scenario){
    let success = !scenario.is_phish;
    let basePoints = scenario.is_phish ? -50 : 20;
    processAction(scenario, success, basePoints);
}

function reportEmail(){
    const scenario = scenarios[currentScenarioIndex];
    let success = scenario.is_phish;
    let basePoints = scenario.is_phish ? 100 : -20;
    processAction(scenario, success, basePoints);
}
...
function updateStreakUI() {
    ...
    if(streakEl) {
        if(currentStreak > 1) {
            streakEl.innerHTML = `🔥 Streak: x${currentStreak}`;
